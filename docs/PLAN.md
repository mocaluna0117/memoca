...
| ドメイン | 当面 `memoca-app.vercel.app`（`memoca.vercel.app` は使用済）。独自ドメインへ移る場合、生体認証ロックは全員再登録（パスワードで解除できるためデータは失われない） |
| ロック | E2EE。ロック対象の本文・タイトル・添付を端末側で AES-256-GCM 暗号化。解除は金庫パスワード（Argon2id）/ Face ID・Touch ID・端末ロック（WebAuthn PRF）/ リカバリーキー |
...
- 鍵階層: 金庫鍵 VK（256bit 乱数。メモリ上は non-extractable CryptoKey）を 3 系統の KEK でラップして `vaults` に保存。
  - パスワード: Argon2id(m=64MiB, t=3, p=1, hash-wasm。中級スマホで 1〜2 秒。WASM は単一スレッドなので p>1 は無意味。パラメータは保存し将来引き上げ可) → KEK
  - リカバリーキー: 256bit 乱数を Base32 8×5 文字で 1 度だけ表示（一部を打ち戻させて確認）→ HKDF → KEK
  - パスキー: WebAuthn PRF（`eval.first = prfInput`, `userVerification: "required"`）→ HKDF-SHA256 → KEK。資格情報ごとに 1 エントリ（iCloud 同期パスキーは Apple 端末間で同じ PRF を返す。Android / Windows は端末ごとに登録）。登録時に `prf.enabled` を確認し、非対応ならパスワードのみ
...
- **ロック（オンライン時のみ）**: ①そのメモの outbox を送り切る ②平文添付をダウンロードし DEK で暗号化して新 storageId にアップロード ③ `vault.lockNote({noteId, keyEpoch: n+1, wrappedKey, titleCt, snapshotCt, coversThroughSeq, attachments[]})`。サーバーは `coversThroughSeq == lastUpdateSeq` を要求し、同一 mutation で平文の更新・スナップショット・title・preview・旧添付ファイルを全削除して `locked, keyEpoch` を設定。解除は正確に逆手順。古い平文 op を持つ別端末の push は `keyEpoch` 不一致で拒否され、その端末は snapshot を取得・復号 → ローカル Y.Doc に適用 → `encodeStateAsUpdate(local, stateVector(snapshot))` の差分を新 epoch で暗号化して再送（金庫解除が必要なので解除まで保留）。
- **フォルダロック**: `folders.locked` は方針、`notes.locked` は実体。フォルダロック = フラグ設定後、配下メモを順に `lockNote`（進捗 UI）。起動時に「ロック済フォルダ内に平文メモが残っている」状態を検出して修復（途中で中断されたカスケード対策）。ロック済フォルダ内の新規メモは最初から暗号化（金庫が閉じていれば解除を促す）。平文メモをロックフォルダへ移動 = `lockNote`、ロック済メモを外へ移動 = ロックのまま（解除は明示操作のみ）。
- 一覧では「🔒 ロック中のメモ」プレースホルダ。金庫解除中だけメモリ上で復号（タイトルは解除直後に一括、本文は Worker で抽出）し、検索インデックスにも一時投入。Dexie には常に暗号文のみ。
- 自動ロック: 無操作 N 分（既定 5）または非表示 5 分で VK・DEK・復号済み文書・検索インデックスをメモリから破棄。iOS のプロセス kill でも自然にロック。Face ID は 1 タップで再解除。任意設定「1 時間ロック解除を維持」は non-extractable CryptoKey を有効期限付きで Dexie に置いて実現（生バイトは取り出せない）。
- パスワード変更・リカバリーキー再発行・パスキー再登録は VK の再ラップのみ（`vaults.version` を上げる。本文の再暗号化なし）。どれか 1 つ生きていれば他を再設定できる。全部紛失 → 復元不可（設定時に明示。「ロック済コンテンツを削除して容量を戻す」導線は用意）。
...
- `/app/f/[folderId]`, `/app/n/[noteId]`, `/app/inbox`, `/app/trash`, `/app/search`, `/app/settings`（外観・保持期間・自動ロック・金庫・端末・アカウント削除）, `/app/admin`。
