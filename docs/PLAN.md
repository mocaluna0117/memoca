...
### Phase 7c — 容量対策とデスクトップ版の即席メモ（2026-09-26〜）
画像の容量対策（Safari でも WebP、画像の種類に応じた画質、容量の内訳）、即席メモ画面のスマホ・小窓両用への改修、Tauri による Mac・Windows のデスクトップ版（ホットキー・メニューバー／トレイ・ホットコーナー）を 3 段で進める。あわせて、ロックしたメモにコピーした画像が暗号化されない抜けを先に直す。計画と進捗は [`docs/STORAGE-AND-DESKTOP.md`](STORAGE-AND-DESKTOP.md)。
