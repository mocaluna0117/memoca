...
| S5 | 画像の種類で画質と大きさを選ぶ | 未着手（実機の診断結果を見てから） |
...
### S6 の実装で決めたこと（2026-09-26）

- **内訳のクエリ `usage.breakdown`**：本文（使用中・ゴミ箱）、ファイルの種類別（画像・動画・その他）、ゴミ箱のメモのファイル、使われなくなったファイル（件数と、次に削除される日時）、送信中、行から数え直した合計、大きい順 20 件（使っているメモ付き）。ゴミ箱に入れたフォルダの中のメモもゴミ箱として数え、ほかのメモも使っているファイルはゴミ箱に数えない（ゴミ箱を空にしても減らないため）。全行を読むので、**購読せず、開いたときに 1 回だけ取る**こと（購読すると編集のたびに全件を読み直し、無料枠の読み取り量をすぐに使い切る。レビューでの試算）。
- **完全削除したファイルはトゥームストーンにする**：ゴミ箱を空にしたとき、ファイルの行を消さずに「削除済み」として残し、端末に同期する（端末は自分の行とキャッシュを消す）。名前・種類・寸法・暗号化した名前と鍵は残さない。90 日後に消す（300 件ずつ、残りがあれば続けて）。
- **ほかのメモが使っているかもしれないファイルは、すぐには消さない**：そのメモからの「使っているファイル」の報告がまだ届いていないときは、ゴミ箱を空にしてもファイルを残し、全メモの報告がそろってから毎日の掃除が消す（消すとその画像がすべての端末から消えるため）。
- **アップロード中にメモを完全削除したら**、あとから届いたアップロードは受け取らず、確保していた容量を戻す（受け取ると、使用量が戻らなくなっていた）。
- **再計算（管理者用）**は削除済みのファイルを数えない。行を読み切れないときは書き込まない。再試行したアップロードが、確保した容量を二重に戻すこともなくした。
- ファイルの行に、アップロード時に端末が申告した種類（`category`）を残す（ロックしたファイルも種類が分かる）。

### S7 の実装で決めたこと（2026-09-27）

- **設定の「保存容量」に内訳**：帯グラフと一覧で、画像・動画・ロックされたファイル（種類を記録する前のもの）・その他のファイル・メモの本文・ゴミ箱・使われなくなったファイル（件数と、削除が始まる日）・送信中・空き。空きは上の「使用中」と同じ最新の数字から出し、帯と一覧を足すと上の使用量になる（数え直しとの差は「内訳に含まれない分」として見せる）。
- **取り方**：内訳はアカウントの全行を読むので、ページを開いたときと［更新］のときだけ取り、5 分以内なら前の結果を使い回す（「HH:mm 時点」と表示）。オフラインではそう伝え、つながったら取る。15 秒で打ち切り、失敗したら前の数字を残したまま伝える。
