...
| 画像 / 動画 | 画像は端末側で WebP に圧縮し、貼ったあとでトリミングも可能。動画はアップロードと URL 埋め込みの両対応 |
| 即席メモ | Inbox にワンタップで書き捨て、後から整理 |
| ロック | 端末側で暗号化する E2EE。パスワード / Face ID・Touch ID / リカバリーキーで解除 |
| ゴミ箱 | ソフト削除と復元。保持期間を過ぎたものは自動で完全削除 |
| 検索 | フォルダ名・メモ名に加えて本文も検索。表記ゆれを吸収し、読み（やっきょく→薬局）でも探せます。⌘K でも開く |
...
公開中: **https://memoca-app.vercel.app**

パソコンのブラウザと、iPhone のホーム画面に追加した状態の両方で動作を確認済みのもの。
...
- **Face ID でのロック解除**。iOS のホーム画面アプリでも動作します
- ゴミ箱への移動と復元、完全削除
- 本文を含む検索、⌘K のコマンドパレット
- **読みでの検索**。「薬局」を「やっきょく」で見つけられます（設定から有効化）
